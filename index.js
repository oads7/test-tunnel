...
const ultraTunnel = () => {
  ...
  // In case of error, report and close
  tunnel.on('error', error => {
    console.error('<tunnel_error>: ', error);
    tunnel.close();
  });

  // In case of tunnel closed, report and close sockets
  tunnel.on('close', () => {
  ...
  tunnel.on('listening', () => {
    ...
    console.log(`<tunnel_listening> on ${address}:${port} (${family})`);
  });

  // In case of a connection, handle it
  tunnel.on('connection', socket => {
    // Report connection
    const nSocket = socketList.push(socket) - 1;
    console.log(`<tunnel_connection> socket ${nSocket + 1}: connected`);

    // In case of the socket has error report
    socket.on('error', error => {
      console.error(`<socket_error> socket ${nSocket + 1}: `, error);
      socket.destroy();
    });

    // In case of the socket is closed
    socket.on('close', () => {
      socketList.splice(nSocket, 1)
      console.log(`<socket_close> socket ${nSocket + 1} closed`);
    });

    // In case of data stream, handle it
    socket.on('data', buffer => {
      httpParse(buf);
        ...
        server.on('connection', socketServer => {
            socketServer.write(buffer)
            socketServer.on('data', response => {
      ...
      if (header !== 'HEAD') console.log('ahh')
